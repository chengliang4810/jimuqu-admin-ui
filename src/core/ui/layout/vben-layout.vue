...
const getSideCollapseWidth = computed(() => {
  const { sidebarExtraCollapsedWidth, sideCollapseWidth } = props;

  return isSidebarMixedNav.value || isHeaderMixedNav.value
...
<template>
  <div class="relative flex min-h-full w-full overflow-x-hidden">
    <LayoutSidebar
    ...
    <div
      ref="contentRef"
      class="flex min-w-0 flex-1 flex-col overflow-hidden transition-all duration-300 ease-in"
    >
      <div
        :class="[
          {
            'shadow-[0_16px_24px_var(--ant-color-bg-base)]': scrollY > 20,
          },
          SCROLL_FIXED_CLASS,
        ]"
