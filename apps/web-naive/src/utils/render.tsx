 ...
 * @returns render
 */
function renderTag(text: string, type?: TagType) {
...
export function renderHttpMethodTag(type: string) {
  ...
  const typeResult = typeMap[method] ?? 'default';
  const title = `${method}请求`;

  return <NTag type={typeResult}>{title}</NTag>;
}

export function renderDictTag(value: number | string, dicts: Dict.DataVo[]) {
